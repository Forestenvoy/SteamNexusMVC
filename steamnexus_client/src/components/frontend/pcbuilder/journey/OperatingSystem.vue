<template>
  <CRow>
    ...
    <CCol xs="12" md="6" class="mb-5 mb-md-0 d-flex flex-column justify-content-center">
      <!-- 標題 -->
      <h3 class="title text-center">作業系統</h3>
      <!-- 圖片 -->
      <div class="d-flex justify-content-center align-items-center">
        <img src="@/assets/images/builder/os.png" alt="OS" class="image" />
      </div>
    </CCol>
    <!-- 右側 -->
    <CCol xs="12" md="6">
...
// (Async) get OS Data
const getData = async () => {
  console.log('Get OS Data...')
  await fetch(`${apiUrl}/api/PcBuilder?type=10010`, {
    method: 'GET'
  })
    .then((response) => {
      if (!response.ok) {
...
const classification = (data) => {
  data.forEach((item) => {
    ...
    if (!OSGroups.value[className]) {
      OSGroups.value[className] = []
    }
    OSGroups.value[className].push(item)
  })
  // 將資料存入 session storage
  sessionStorage.setItem('OSList', JSON.stringify(OSGroups.value))
  SortGroups.value = OSGroups.value
}
...
const filterByLanguage = () => {
  if (language.value !== 'All') {
    // 過濾名稱內含有該品牌的資料
    const filteredGroups = Object.keys(OSGroups.value).reduce((result, key) => {
      const filteredProducts = OSGroups.value[key].filter((product) =>
        product.name.includes(language.value)
      )
...
const selectedProduct = (event) => {
  ...
  // 加入至產品清單
  builderStore.addProduct('OS', Product)
}

onMounted(() => {
  // 檢查 session storage 是否有資料
  const storedData = sessionStorage.getItem('OSList')
  if (storedData !== null && storedData !== undefined) {
    OSGroups.value = JSON.parse(storedData)
    SortGroups.value = OSGroups.value
  } else {
    getData()
  }
