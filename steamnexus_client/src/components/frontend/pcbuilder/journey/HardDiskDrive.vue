<template>
  <CRow>
    ...
    <CCol xs="12" md="6" class="mb-5 mb-md-0 d-flex flex-column justify-content-center">
      <!-- 標題 -->
      <h3 class="title text-center">機械硬碟</h3>
      <!-- 圖片 -->
      <div class="d-flex justify-content-center align-items-center">
        <img src="@/assets/images/builder/hdd.png" alt="HDD" class="image" />
      </div>
    </CCol>
    <!-- 右側 -->
    <CCol xs="12" md="6">
      ...
      <CRow class="mb-5">
        <CCol xs="12" class="d-flex justify-content-center align-items-center">
          ...
          <!-- SSD 可以有複數項 -->
          <div tabindex="0" class="plusButton ms-3" @click.stop="addHDD">
...
const filterByBrand = () => {
  if (brand.value !== 'All') {
    // 過濾名稱內含有該品牌的資料
    const filteredGroups = Object.keys(HDDGroups.value).reduce((result, key) => {
      const filteredProducts = HDDGroups.value[key].filter((product) =>
...
const addHDD = () => {
  if (Product.id !== undefined) {
    builderStore.addProduct('HDD', Product)
  }
}
