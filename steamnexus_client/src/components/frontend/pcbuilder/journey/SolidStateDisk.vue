<template>
  <CRow>
    ...
    <CCol xs="12" md="6" class="mb-5 mb-md-0 d-flex flex-column justify-content-center">
      <!-- 標題 -->
      <h3 class="title text-center">固態硬碟</h3>
      <!-- 圖片 -->
      <div class="d-flex justify-content-center align-items-center">
        <img src="@/assets/images/builder/ssd.png" alt="SSD" class="image" />
      </div>
    </CCol>
    <!-- 右側 -->
    <CCol xs="12" md="6">
      ...
      <CRow class="mb-5">
        <CCol xs="12" class="d-flex justify-content-center align-items-center">
          ...
          <!-- SSD 可以有複數項 -->
          <div tabindex="0" class="plusButton ms-3" @click.stop="addSSD">
...
// 品牌清單
const brandList = [
  { name: 'KLEVV', value: 'KLEVV' },
  { name: 'Samsung 三星', value: '三星' },
  { name: 'Seagate 希捷', value: 'Seagate' },
  { name: 'Solidigm', value: 'Solidigm' },
  { name: 'UMAX', value: 'UMAX' },
  { name: 'WD', value: 'WD' },
  { name: '金士頓', value: '金士頓' },
  { name: '技嘉', value: '技嘉' },
  { name: '十銓', value: '十銓' },
...
const filterByBrand = () => {
  if (brand.value !== 'All') {
    // 過濾名稱內含有該品牌的資料
    const filteredGroups = Object.keys(SSDGroups.value).reduce((result, key) => {
      const filteredProducts = SSDGroups.value[key].filter((product) =>
...
const addSSD = () => {
  if (Product.id !== undefined) {
    builderStore.addProduct('SSD', Product)
  }
}
...
.plusButton::before {
  ...
  border-style: solid;
  border-color: transparent #f3ae0b transparent transparent;
